...
class Pos:
    ...
    def draw(self, win):
        pg.draw.rect(win, self.color, (self.x, self.y, self.width, self.width)) #square --> width = height
...
def algorithm(draw, grid, start, end):
    ...
    open_set.put((0, count, start))    # put = add to PQ
    last = {}
    ...
    while not open_set.empty():
        ...
        if current == end:
            ...
            start.make_start()
            end.make_end() # no path on end node
            return True
...
def main(win, width):
    ...
    while run:
        ...
        for event in pg.event.get():
            ...
            if pg.mouse.get_pressed()[0]:   # leftMouseClick
                ...
                if not start and spot != end:
                    ...
                    spot.make_barrier()
            elif pg.mouse.get_pressed()[2]: # rightMouseClick
                pos = pg.mouse.get_pos()
                row, col = get_clicked_pos(pos, ROWS, width)
                spot = grid[row][col]
                spot.reset()
